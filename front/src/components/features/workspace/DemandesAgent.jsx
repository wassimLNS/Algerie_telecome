...
import { getMesDemandes, creerDemande, downloadFichierDemande } from '@/api/demandes';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Plus, Send, X, Clock, CheckCircle, XCircle, Loader2, AlertTriangle, Download } from 'lucide-react';
...
const PRIORITE_OPTIONS = [
  { value: 'basse', label: 'demands_priority.basse', color: 'text-slate-500' },
  { value: 'normale', label: 'demands_priority.normale', color: 'text-blue-600' },
  { value: 'haute', label: 'demands_priority.haute', color: 'text-orange-600' },
  { value: 'urgente', label: 'demands_priority.urgente', color: 'text-red-600' },
];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-black uppercase tracking-tighter text-slate-900">{t('demands.title_agent')}</h2>
        <Button onClick={() => setShowForm(!showForm)} className="bg-[#0055A4] hover:bg-[#004080] text-white font-bold rounded-xl px-6">
          {showForm ? <X className="w-4 h-4 mr-2" /> : <Plus className="w-4 h-4 mr-2" />}
          {showForm ? t('common.cancel') : t('demands.new_demand')}
        </Button>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder={t('demands.subject_placeholder')}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t('demands.description_placeholder')}
                  rows={4}
